  ...
  <div class="singleLevel common" v-loading="loading">
    ...
    <div class="content-box" v-for="item in list" :key="item.title">
      ...
      <div class="content-box-content">
        ...
        <p>优秀: 满分的 {{ item.rankRule.gradeOne.toFixed(2) }}% 及以上</p>
        <p>良好: 满分的 {{ item.rankRule.gradeTwo.toFixed(2) }}% 及以上</p>
        <p>及格: 满分的 {{ item.rankRule.gradeThree.toFixed(2) }}% 及以上</p>
      </div>
          ...
          <td>
            <section v-for="item in sectionAddGradeList" :key="item.name" class="dialog-table-section">
              ...
              <p class="dialog-table-checkbox">
                <el-checkbox v-model="items.checked" v-for="(items, i) in item.gradeList" :key="i" @change="getCheckState(items, item)">{{ items.gradeName }}</el-checkbox>
              </p>
            </section>
          </td>
        </tr>
  ...
  methods: {
    ...
    saveDetails () {
      ...
      if (this.oneLevel <= 0 || this.oneLevel > 100) {
        notice(this, '优秀请输入0-100之间的数字', 2000)
        return
      }
      if (this.twoLevel <= 0 || this.twoLevel > 100) {
        notice(this, '良好请输入0-100之间的数字', 2000)
        return
      }
      if (this.threeLevel <= 0 || this.threeLevel > 100) {
        notice(this, '及格请输入0-100之间的数字', 2000)
        return
      }
