<template>
  <div class="total-sum common" v-loading="loading">
        ...
        <p>
          ...
          <section>
            <p>
              ...
              <span class="sublist">
                <span v-for="items in (item.courses.filter(item => item.requireFlag === 1))" :key="items.courseName">{{ items.courseName }} </span>
              </span>
            </p>
            <p>
              <span class="green span">选考科目</span>
              <span class="sublist">
                <span v-for="items in  (item.courses.filter(item => item.requireFlag === 2))" :key="items.courseName">{{ items.courseName }} </span>
              </span>
            </p>
          </section>
        </p>
        ...
        <tr>
          ...
          <td>
            <section v-for="(item, index) in sectionAddGradeList" :key="item.name" class="dialog-table-section">
              <p>
                <el-radio v-model="checkedAll" :label="index">{{ item.name }}</el-radio>
              </p>
...
export default {
  ...
  data () {
    return {
      ...
      ruleTypeList: [
        { name: '总分相加', type: 1 },
        { name: '系数折算', type: 2, disabled: true }
      ],
      asideList: [],
      examList: [],
      singleId: '',
  ...
  methods: {
    ...
    async getSection (s) {
      ...
      .then(res => {
        if (res.data.code === 1) {
          let result = (res.data.result || []).filter(item => item)
      ...
      .then(res => {
        ...
        if (res.data.code === 1) {
          ...
          res.data.result.forEach(item => {
            this.mustCourse = item.courses.filter(items => items.requireFlag === 1)
            this.chooseCourse = item.courses.filter(items => items.requireFlag === 2)
          })
    ...
    saveDetails () {
      ...
      if (this.chooseList.length < 3 && this.chooseList.length > 0) {
        notice(this, '如果有选考科目应至少为三科', 2000)
        return
      }
    ...
    async editDetail () {
      ...
      .then(async res => {
        if (res.data.code === 1) {
          ...
          this.sectionAddGradeList.map(item => ({
            ...item,
            gradeList: item.gradeList.map(items => ({
              ...items,
              checked: false
  ...
  watch: {
    checkedAll (val) {
      ...
      if (!this.sectionAddGradeList[val]) return
