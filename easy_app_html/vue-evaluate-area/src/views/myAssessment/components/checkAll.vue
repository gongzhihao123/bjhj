    ...
    <div class="content">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="总分">
          <template scope="scope">
            <span :class="{ active: scope.row.userId === user.id }">{{scope.row.totalScore === 0 ? scope.row.totalScore : scope.row.totalScore.toFixed(2)}}</span>
          </template>          
        </el-table-column>
        <el-table-column
          width="18%"
        ...
        <el-table-column :label="mode.modeName" width="18%" v-for="(mode, i) in modeList" :key="i">
          <template scope="scope">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="单位">
          <template scope="scope">
            <span :class="{ active: scope.row.userId === user.id }">{{scope.row.companyName}}</span>  
          </template>     
        </el-table-column>
        <el-table-column
          width="15%"
        ...
        .then(res => {
          if (res.data.code === 1) {
            ...
            this.tableData.forEach(item => {
              // 遍历，比较item.scoreList.length和modeList.length的长度
              if (item.scoreList.length !== this.modeList.length) {
                let num = item.scoreList.length
                for (let i = 0; i < (this.modeList.length - num); i++) {
