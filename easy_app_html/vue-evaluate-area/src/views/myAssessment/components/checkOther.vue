    ...
    <div class="content">
      ...
      <table class="table-box" border="1" v-if="scoreList.length !== 0">
        ...
        <tr v-for="(list, index) in scoreList" :key="index">
          <th>{{list.modeName}}</th>
          <th>{{list.score === 0 ? list.score : list.score.toFixed(1)}}</th>
          <th v-if="index === 0" :rowspan="scoreList.length"><span>{{totalScore === 0 ? totalScore : totalScore.toFixed(2)}}</span></th>
        </tr>
      </table>
      <p v-if="scoreList.length === 0" class="null">暂无数据</p>
    </div>         
  </div>
</template>
