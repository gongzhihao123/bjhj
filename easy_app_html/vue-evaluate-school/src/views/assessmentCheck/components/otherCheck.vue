        ...
        <div class="button-group">
          ...
          <p class="returnbox" v-if="rowIn.checkState === 3">
            <ul>
              ...
              <li class="returnReason"><span v-if="!rowIn.reason">无</span><span>{{rowIn.reason}}</span></li>
            </ul>
          </p>
            ...
            this.tableData.forEach((item) => {
              ...
              if (item.teachers.length !== 0) {
                ...
                item.students.forEach((student) => {
                  item.checkN.push(student.gradeName + '-' + student.className)
                })
              }
              // 被测评人
              item.users.forEach((user) => {
                item.checkBN.push(String(user.deptName) + '-' + String(user.userName))
              })
