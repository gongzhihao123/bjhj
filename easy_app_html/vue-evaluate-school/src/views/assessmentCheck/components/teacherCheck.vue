<template>
    ...
    <!--问卷-->
      <div class="naire">
        ...
        <ul>
          <li><p>测评类型：</p><span v-if="rowIn.evalType === 1">任课教师测评</span><span v-if="rowIn.evalType === 2">班主任测评</span><span v-if="rowIn.evalType === 3">其他测评</span></li>
...
export default {
  ...
  methods: {
    ...
    // 查询任课教师和班主任测评
    getEvalDetailByTeacher () {
      this.$https.get(`/examine/getEvalDetailByTeacher?evalId=${this.rowIn.id}`)
      .then(res => {
        if (res.data.code === 1) {
          this.naire = res.data.result
    ...
    getEvalTableDeatil () {
      if (this.$store.state.checkState === '1') {
        if (this.rowIn.evalType === 3) {
          this.$https.get(`/common/getEvalTableNeedCheckDeatil?tableId=${this.tableId}`)
          .then(res => {
            if (res.data.code === 1) {
            // 把题型数组合并
              this.newquestion = res.data.result
              // this.question = res.data.result.questions.concat(res.data.result.titles)
              // this.question.forEach((item, index) => {
              // // 赋值 以item.number 为index的新题型数组。
              //   this.$set(this.newquestion, item.number, item)
              // })
          ...
          this.$https.get(`/common/getEvalTableNeedCheckDeatil?tableId=${this.naire.tableId}`)
            .then(res => {
              if (res.data.code === 1) {
                // 把题型数组合并
                this.newquestion = res.data.result
        ...
        if (this.rowIn.evalType === 3) {
          ...
          this.$https.get(`/common/getEvalTableDeatil?tableId=${this.naire.tableId}`)
            .then(res => {
              if (res.data.code === 1) {
                // 把题型数组合并
                this.newquestion = res.data.result
