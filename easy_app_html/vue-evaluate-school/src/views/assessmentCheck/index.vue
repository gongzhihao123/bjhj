            ...
            <el-table-column
                ...
                <template scope="scope">
                  <el-popover
                    ...
                    >
                  <span v-if="scope.row.reason && scope.row.reason.length >= 10" slot="reference">{{scope.row.reason.slice(0, 10) + '...'}}</span>
                  </el-popover>                
                  <span v-if="scope.row.reason && scope.row.reason.length < 10">{{scope.row.reason}}</span>                  
...
export default {
  ...
  methods: {
    ...
    CheckIn (index, row) {
      ...
      if (row.evalType !== 3) {
        // 任课教师和班主任
        this.$router.push({path: '/assessmentCheck/teacherCheck'})
