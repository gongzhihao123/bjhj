      ...
      <transition-group name="slide-left" mode="out-in">
        <el-row @mouseleave.native="item.show = false" class="list-body" v-for="(item, index) in listBody" :key="index">
          ...
          <el-col :span="6">
            ...
            <span @click="lookDetails(item)" v-if="item.name.length < 10" style="cursor: pointer;">{{ item.name }}</span>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    lookDetails (item) {
      ...
      this.$router.push({path: `/assessmentManager/lookDetails`, query: {parameter: JSON.stringify(parameter)}})
    },
    // 提交审核
    submitCheck (item) {
    ...
    edit (item) {
      ...
      // this.$router.push('/assessmentManager/editAssessment')
    },
    // 创建测评
    createAssessment () {
  ...
  created () {
    ...
    if (this.$route.query.parameter) {
      this.pageNo = JSON.parse(this.$route.query.parameter).pageNo
      this.pageSize = JSON.parse(this.$route.query.parameter).pageSize
    }
    this.getList()
  }
}
