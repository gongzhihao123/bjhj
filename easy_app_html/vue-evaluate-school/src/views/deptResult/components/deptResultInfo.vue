      ...
      <el-table
        ...
        <el-table-column
            ...
            <template scope="scope">
              <span>{{ scope.row.totalScore === 0 ? scope.row.totalScore : scope.row.totalScore.toFixed(1) }}</span>
            </template>
        </el-table-column>
        <el-table-column
            prop="sort"
...
export default {
  ...
  methods: {
    ...
    handleSizeChange (val) {
      this.pageSize = val
      if (this.dept.evalType === 3) {
        this.getDeptEvalDetailByOther(val)
      } else {
        this.getDeptEvalDetailByTeacher(val)
      }
    },
    handleCurrentChange (val) {
      ...
      if (this.dept.evalType === 3) {
        ...
        this.getDeptEvalDetailByTeacher()
      }
    },
    getDeptEvalDetailByTeacher () {
      this.loading = true
      setTimeout(() => {
        this.$https.get(`deptEvaluate/getDeptEvalDetailByTeacher?evalId=${this.dept.id}&pageNo=${this.currentPage}&pageSize=${this.pageSize}`)
        .then(res => {
          if (res.data.code === 1) {
