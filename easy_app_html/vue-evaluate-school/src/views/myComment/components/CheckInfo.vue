<template>
    ...
    <!--问卷-->
      <div class="naire">
        ...
        <ul>
          <li v-if="rowInfo.evalType === 3"><span class="tips"><img src="../../../assets/images/person.png"><i>/</i>被测评人：</span>{{rowMes.userName}}</li>
          <li v-if="rowInfo.evalType !== 3"><span class="tips"><img src="../../../assets/images/person.png"><i>/</i>被测评人：</span>{{rowMes.teacherName}}</li>
          ...
          <li v-if="rowInfo.evalType === 3"><span class="tips"><img src="../../../assets/images/dept.png"><i>/</i>部门/职务：</span>{{rowMes.deptName}}</li>
...
export default {
  ...
  methods: {
    ...
    back () {
      ...
      if (this.rowInfo.evalType === 2) {
        this.$router.push({path: '/myComment/CommentTeacher'})
      } else if (this.rowInfo.evalType !== 2) {
        if (this.groupstate === 1 && this.rowInfo.evalType === 3) {
          this.$router.push({path: '/myComment/otherComment'})
        } else if (this.groupstate !== 1 && this.rowInfo.evalType === 3) {
          this.$router.push({path: '/myComment/groupInfo'})
        }
        if (this.groupstate === 1 && this.rowInfo.evalType === 1) {
          this.$router.push({path: '/myComment/CommentName'})
        } else if (this.groupstate !== 1 && this.rowInfo.evalType === 1) {
          this.$router.push({path: '/myComment/CommentName'})
        }
      }
    },
    submit () {
      ...
      if (a.length !== 0) {
        ...
        // 提交--完全答题
        this.$https.post(`myEvaluate/saveEvalUserResult`, querystring.stringify({eosIds: this.eosIds, questionTypes: this.questionTypes, answers: this.answerstr, evalId: this.rowMes.evalId, childName: this.childName}))
        .then(res => {
          if (res.data.code === 1) {
    ...
    getEvalUserForTableAndResult () {
      ...
      if (this.rowInfo.evalType === 3) {
        this.answers = []
        this.answerstr = ''
        this.$https.get(`myEvaluate/getEvalUserForTableAndResult?evalId=${this.rowMes.evalId}&groupId=${this.rowMes.groupId}&userId=${this.rowMes.userId}&tableId=${this.rowMes.tableId}&subjectId=${''}&childId=${this.childId}`)
        .then(res => {
          if (res.data.code === 1) {
        ...
        this.answers = []
        if (this.rowMes.subjectId === null) {
          this.rowMes.subjectId = ''
        }
        this.$https.get(`myEvaluate/getEvalUserForTableAndResult?evalId=${this.rowMes.evalId}&userId=${this.rowMes.teacherId}&tableId=${this.rowMes.tableId}&subjectId=${this.rowMes.subjectId}&groupId=${''}`)
        .then(res => {
          if (res.data.code === 1) {
            this.newquestion = res.data.result[0].tableTeacherDtoList.map(item => ({...item, textarea: '', radio: ''}))
            this.eosIds = this.newquestion.map(item => item.etsId).join(',')
            this.questionTypes = this.newquestion.map(item => item.questionType).join(',')
            this.newquestion.forEach(item => {
              // 选择题
              if (item.content) {
