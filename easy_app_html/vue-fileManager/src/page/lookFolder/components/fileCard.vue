<template>
  <div class="fileCard" ref='fileCard' @click="closeFold">
    <!-- <div class="boxSelects" v-if="selctBox"></div> -->
    ...
    <div class="folderCard">
      <div class="cardfold" v-for="(item, index) in filesList" :key="index" @dblclick="addTr(item)" @click.stop="addSelect(item)" @contextmenu.prevent="menutext($event, item, index)" :class="item.show ? 'selectThe' : ''" ref="selectFiles">
        ...
        <span v-if="!item.rename">{{ item.name.length > 3 ? item.name.slice(0, 3) + '...' + (item.fileSuffix ? item.fileSuffix : '') : item.name }} <br> {{  item.userName }} </span>
        <el-input v-else placeholder="请输入文件名称" :autofocus="true" @keydown.enter.native="renameFolder" v-focus="true" @blur="renameFolder" v-model="folderNames.name"></el-input>
      </div>
       <div v-if="!addFold" class="addcard">
        <img src="./../../../assets/image/folder-list.png">
        <el-input v-focus :autofocus="true"  @keydown.enter.native="addFoldNmae" @blur="addFoldNmae" v-model="folderName" placeholder="默认文件夹"></el-input>
      </div>
    </div>
    ...
    width="500px">
        ...
        <div class="Readwrite">
          ...
          <transition name="list" mode="out-in">
            <div class="treeUserBody" v-if="treeListView">
              <div class="treeUser">
                <div>
                  <el-input v-focus="true" v-model="userFilter" placeholder="请输入关键字"></el-input>
                  <el-tree
                    :data="userTree"
                    node-key="userId"
                    show-checkbox
      ...
      <div slot="footer">
        <el-button @click.stop="userPermissions = false; selctBox = true; radio = '1'">取消</el-button>
...
export default {
  ...
  methods: {
    ...
    selctBoxs () {
      // const _this = this
      // this.chooseBox = new BoxChoose('.boxSelects', (item) => {
      //   if (item.move) {
