    ...
    <div class="create-details">
      ...
      <div class="details-content">
        ...
        <div class="on-right">
          ...
          <!-- <p class="title" v-show="!item.modState">
            <p>{{item.title}}</p>
          </p> -->
          ...
          <el-button type="success" class="save-created" v-show="questionList.length != 0" @click="saveAll">完成并发布</el-button>
    ...
    <el-dialog
      ...
      <div class="dialog-content">
        ...
        <section class="center section">
          <ul>
            <li v-for="(item, i) in templateList" :class="{active: temInd == i}" :key="i" @click="switchTem(item, i)">{{ item.title | textLength }}</li>
          </ul>
        </section>

        <!--右-->
        <section class="right section" v-if="!preview">
...
export default {
  ...
  created () {
    ...
    this.naireId = sessionStorage.naireId
    // console.log(this.naireId)
  ...
  methods: {
    ...
    blur () {
      ...
      let questionJson = JSON.stringify(this.questionList.map(item => ({...item, naireId: this.naireId})))
    ...
    async titleBlur () {
      ...
      this.naireId = sessionStorage.naireId
      if (!this.naireId) {
        let naireJson = sessionStorage.naireJson
        await this.$https.post('/publish/naireSave', querystring.stringify({
          naireJson
        }))
        .then(async res => {
          if (res.data.code === 1) {
            this.naireId = res.data.result
            sessionStorage.naireId = this.naireId
            await this.blur()
            // console.log(2)
    ...
    saveCreated (item, s) {
      return new Promise(async (resolve, reject) => {
        ...
        this.questionList.forEach(items => {
          if (items.questionType === 4) {
            this.$set(items, 'sorts', items.sort)
          } else {
            this.$set(items, 'sort', items.sorts)
          }
        })
        let jsonstr = []
        ...
        if (!this.naireId) {
          ...
          .then(async res => {
            if (res.data.code === 1) {
              this.naireId = res.data.result
              sessionStorage.naireId = this.naireId
              this.questionList[this.modActive || 0].naireId = this.naireId
              let questionJson = JSON.stringify([this.questionList[this.modActive || 0]])
    ...
    getTemplateList () {
      ...
      .then(res => {
        if (res.data.code === 1) {
          ...
          // console.log(0)
          this.switchTem(this.templateList[0], 0)
        } else {
          notice(this, res.data.message, 2000)
        }
      })
    ...
    // 公开问卷发布
    async publishNaire () {
      await this.$https.get('common/initPage')
      .then(res => {
        if (res.data.code === -2) {
    ...
    previewOpen (s) {
      ...
      this.previewData = {
        ...
        qNum: this.questionList.filter(item => item.questionType !== 4).length
      }
      // 查看
      // if (s === 1) {
