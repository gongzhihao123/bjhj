...
export default {
  ...
  methods: {
    ...
    deptListClick (data, val) {
      ...
      // if (data.userId) {
      if (Number(this.state) === 3 && Number(this.people) === 1) {
        console.log(1)
        this.deptUserList = this.$refs.tree.getCheckedNodes(true)
      } else if (Number(this.state) === 2 && (Number(this.typeList[this.types]) === 2 || Number(this.typeList[this.types]) === 3)) {
        this.deptUserList = this.$refs.tree.getCheckedNodes(true).filter(item => !item.clazzName)
        console.log(2)
      } else {
        this.deptUserList = this.$refs.tree.getCheckedNodes(true).filter(item => item.userId && item.username)
        console.log(3)
      }
      this.checkedCount = this.deptUserList.length
      // }
