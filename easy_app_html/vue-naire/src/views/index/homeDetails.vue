        ...
        <div v-if="item.questionType == 1">
          ...
          <ul>
            <li v-for="(items, index) in item.options" :key="index">
              ...
              <span :class="{num: true, check: showResult == 1}" @click="clickOption(items, item)">{{items.radioboxCount ? items.radioboxCount : 0}}票</span>
              <span class="progress">
        ...
        <div v-if="item.questionType == 2">
          ...
          <ul>
            <li v-for="(items, index) in item.options" :key="index">
              ...
              <transition name="fade" v-show="showResult == 1">
                <section class="content" v-show="items.show">
                  <span v-for="s in items.answerList" :key="s" :class="{check: showResult == 1}" @click="route(s)">{{s.user_name}}</span>
                </section>
              </transition>
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    // 查看问卷结果
    detail () {
      this.$https.get(`/index/naireAnswerDetail?naireId=${this.naireId}&childId=${this.$route.query.childId || ''} `)
      .then(res => {
        if (res.data.code === 1) {
    ...
    naireAnswerSituation () {
      this.previent = true
      this.$https.get(`/index/naireAnswerSituation?naireId=${this.naireId}&childId=${this.$route.query.childId || ''}`)
      .then(res => {
    ...
    naireAnswerUserDetail (item, items) {
      this.$https.get(`/index/naireAnswerUserDetail?questionId=${item.id}&optionId=${this.optionsId}&naireId=${this.naireId}&childId=${this.$route.query.childId || ''}`)
      .then(res => {
        if (res.data.code === 1) {
          // console.log(item)
    ...
    route (item) {
      let path = document.querySelector('#contextPath').value
      window.open(`${path}/#/answerCheck?naireId=${this.naireId}&userId=${item.user_id}`)
      // this.$router.push({path: '/answerCheck', query: {naireId: this.naireId, userId: item.user_id}})
    },
    router () {
      this.$router.push({path: '/index', query: {page: this.$route.query.page}})
    }
  },
  created () {
