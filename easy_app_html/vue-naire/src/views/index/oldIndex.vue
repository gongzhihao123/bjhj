<template>
  <div id="index">
    ...
    <article :class="{list: isList}" v-if="isList"  v-show="naireList.length != 0">
        ...
        <el-row v-for="(item, i) in naireList" :key="i">
          ...
          <el-col :span="2">
            ...
            <span v-if="item.overtimeFlag && item.type == 3" class="end">已过期</span>
            <span v-if="!item.overtimeFlag && item.type == 3" class="ing">进行中</span>
          ...
          <el-col :span="4" style="overflow: hidden; height: 60px;">
            ...
            <!--外部问卷-->
            <el-button v-if="item.type == 3 && item.answerState == 0 && item.forwardState == 1 && item.showResult == 1" class="blue" @click="otherCheck(item)">查看</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 0 && item.forwardState == 1 && item.showResult == 2" class="blue" @click="otherCheck(item)">查看</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 1 && item.forwardState == 1 && item.showResult == 1 && !item.overtimeFlag" class="blue" @click="otherCheck(item)">查看</el-button>

            <el-button v-if="item.type == 3 && item.answerState == 0 && item.overtimeFlag && item.forwardState == 2" class="blue" @click="otherCheck(item)">查看</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 0 && !item.overtimeFlag && item.forwardState == 2" class="yellow" @click="transpondSure(item)">转发</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 0 && !item.overtimeFlag && item.forwardState == 1" class="yellow" @click="transpondSure(item)" :disabled="true">转发</el-button>
            <!--外部问卷发送给个人-->
            <el-button v-if="item.type == 3 && item.answerState == 1 && !item.overtimeFlag && item.forwardState == 0" class="green" @click="respondence(item, 'answer')">作答</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 1 && !item.overtimeFlag && item.forwardState == 1" class="green" @click="respondence(item, 'answer')">作答</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 1 && !item.overtimeFlag && item.forwardState == 2" class="yellow" @click="transpondSure(item)">转发</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 1 && item.overtimeFlag" class="blue" @click="otherCheck(item)">查看</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 2 &&  item.showResult == 2" class="blue" @click="otherCheck(item)">查看</el-button>
            <el-button v-if="item.type == 3 && item.answerState == 2 &&  item.showResult == 1" class="blue" @click="otherCheck(item)">查看</el-button>
            

          </el-col>
        </el-row>
    </article>
    <article :class="{card: !isList}" v-if="!isList" v-show="naireList.length != 0">
      <div v-for="(item, index) in naireList" :key="index">
        <p v-if="!item.title || item.title.length < 16">
          {{item.title}}
        </p>
        <p v-else class="card-p">
          <el-popover placement="bottom" trigger="hover" :content="item.title">
            <span slot="reference">
              {{item.title | textLength}}
              <span class="type" v-show="item.type === 3">外部问卷</span>
        ...
        <p class="time">截止时间:{{item.endTime}}</p>
        <p class="type">
          <span class="type-left">
            <span class="form" v-if="item.type === 3">外部问卷</span>
          </span>
          <span class="type-right">
            <span v-if="item.joinWay == 1">实名问卷</span>
            <span v-if="item.joinWay == 2">匿名问卷</span>
          </span>
        </p> 
        <!--<p class="state-and-count"><span class="ing" v-if="item.naiveState == 1">进行中</span><span class="count">{{item.count}}份数据</span></p>-->
        <p class="state-and-count">
          <span v-if="item.naireState == 1 && item.type == 2" class="ing">进行中</span>
          ...
          <span class="count">{{item.numCount}}份数据</span>
        </p>
        <p class="card-button">
          <!--公开问卷-->
          <!--公开问卷未答-->
          <el-button v-if="item.type == 1 && item.answerState == 1 && item.naireState == 1" class="green" @click="respondence(item, 'answer')">作答</el-button>
          <el-button v-if="item.type == 1 && item.answerState == 1 && item.naireState == 2 && item.showResult == 1" class="blue"  @click="lookResults(item)">查看结果</el-button>
          <el-button v-if="item.type == 1 && item.answerState == 1 && item.naireState == 2 && item.showResult == 1" class="blue"  @click="lookSelf(item)">查看</el-button>
          <el-button v-if="item.type == 1 && item.answerState == 1 && item.naireState == 2 && item.showResult == 2" class="blue"  @click="lookSelf(item)">查看</el-button>
          <!--公开问卷已答-->
          <el-button v-if="item.type == 1 && item.answerState == 2 && item.showResult == 1" class="blue"  @click="lookResults(item)">查看结果</el-button>
          <el-button v-if="item.type == 1 && item.answerState == 2 && item.showResult == 1" class="blue"  @click="lookSelf(item)">查看</el-button>
          <el-button v-if="item.type == 1 && item.answerState == 2 && item.showResult == 2" class="blue"  @click="lookSelf(item)">查看</el-button>
          ...
          <!--外部问卷-->
          <!--转发给外部单位-->
          <el-button v-if="item.type == 3 && item.answerState == 0 && item.forwardState == 1 && item.showResult == 1" class="blue" @click="lookResults(item)">查看结果</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 0 && item.forwardState == 1 && item.showResult == 1" class="blue" @click="lookSelf(item)">查看</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 0 && item.forwardState == 1 && item.showResult == 2" class="blue" @click="otherCheck(item)">查看1</el-button>

          <el-button v-if="item.type == 3 && item.answerState == 0 && item.overtimeFlag && item.forwardState == 2" class="blue" @click="otherCheck(item)">查看1</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 0 && !item.overtimeFlag && item.forwardState == 2" class="yellow" @click="transpondSure(item)">转发</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 0 && !item.overtimeFlag && item.forwardState == 1" class="yellow" @click="transpondSure(item)">转发</el-button>

          <el-button v-if="item.type == 3 && item.answerState == 1 && item.forwardState == 1 && item.showResult == 1" class="blue" @click="lookResults(item)">查看结果</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 1 && item.forwardState == 1 && item.showResult == 1" class="blue" @click="lookSelf(item)">查看</el-button>
          <!--外部问卷发送给个人-->
          <el-button v-if="item.type == 3 && item.answerState == 1" class="green" @click="respondence(item, 'answer')">作答</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 2 &&  item.showResult == 1" class="blue" @click="lookResults(item)">查看结果</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 2 &&  item.showResult == 1" class="blue" @click="lookSelf(item)">查看</el-button>
          <el-button v-if="item.type == 3 && item.answerState == 2 &&  item.showResult == 2" class="blue" @click="otherCheck(item)">查看</el-button>
        </p>
      </div>
    </article>
    ...
    <el-dialog
      ...
      <p class="p">
        <span>接收人:</span>
        <span value="点击选择接收人" @click="chooseRecipient" class="choose-people">
...
export default {
  ...
  methods: {
    ...
    nodeClick (data) {
      ...
      .then(res => {
        if (res.data.code === 1) {
          ...
          data.userList = result.map(item => ({...item, companyId: data.id, companyName: data.name, userId: item.id}))
          this.$set(data, 'userList', result.map(item => ({...item, companyId: data.id, companyName: data.name, userId: item.id})))
        } else {
          notice(this, res.data.message, 2000)
        }
      })
    ...
    // 转发保存接口
    naireForward () {
      let userjson = JSON.stringify(this.acceptList.map(item => ({userId: item.userId || item.code, userName: item.name || item.userName || item.label, userType: 1})))
    ...
    chooseAuditor () {
      ...
      this.deptUserList.forEach(item => {
        this.asideList.push(item.userName)
      })
    ...
    async getDeplist () {
      ...
      .then(res => {
        if (res.data.code === 1) {
          ...
          this.deptList = result
          // this.deptList = result.map(item => ({...item, checked: false, userType: Number(this.idents), code: item.id, label: item.name}))
          this.filterList = this.copy(this.deptList)
    ...
    // 查看结果
    lookResults (item) {
      this.$router.push({path: '/homeDetails', query: {naireId: item.id, page: this.page}})
    }
  },
  watch: {
    searchTeach (val) {
