<template>
  <div id="answerNaire">
    <answer :details="templatePre">
      <!-- <el-button slot="button" @click="route">返回</el-button> -->
    </answer>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.$https.get(`/manage/naireUserResult?naireId=${this.naireId}&userId=${this.userId}`)
    .then(res => {
      if (res.data.code === 1) {
  ...
  methods: {
    ...
    route () {
      this.$router.push({path: '/details', query: {obj: sessionStorage.obj}})
    }
  }
}
</script>
