<template>
  <div id="lesson-view">
    <!--顶部导航-->
    <header>
      <p>我的&nbsp;> &nbsp;我的备课&nbsp;>&nbsp;{{all.sectionName}}&nbsp;>&nbsp;{{all.gradeName}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;发表时间：
...
export default {
  ...
  methods: {
    ...
    reuse () {
      ...
      sessionStorage.path = '/my/lessonView'
    },
    // 下载课件
    ...
    dblDownloadAttenchment (item, val) {
      ...
      if ((item.fileName.indexOf('docx') !== -1) || (item.fileName.indexOf('ppt') !== -1) || (item.fileName.indexOf('word') !== -1) || (item.fileName.indexOf('pdf') !== -1) || (item.fileName.indexOf('xlsx') !== -1) || (item.fileName.indexOf('xls') !== -1)) {
        window.open(`${path}/square/lessonPlanCourseWareDownload?attachmentId=${item.id}`)
      } else {
    ...
    getDetail (val) {
      ...
      .then(res => {
        if (res.data.code === 1) {
          ...
          this.starValue = res.data.result.plan.score
