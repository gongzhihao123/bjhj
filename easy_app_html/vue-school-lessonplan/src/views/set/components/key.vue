        ...
        <ul v-loading="loading">
          <li v-for="item in setInfo" :key="item">
            <p>
              ...
              <el-checkbox  v-model="item.checkAll" @change="handleCheckAllChange($event, item)" class="selectAll" v-if="item.courses.length !== 0" :indeterminate=item.isIndeterminate>全选</el-checkbox>
              <el-checkbox-group  v-model="item.checked" @change="handlecheckedCoursesChange($event, item)">
              <el-checkbox v-for="subject in item.courses" :label="subject.curriculumName" :key="subject" @change="chooseCourses($event, item)">{{subject.curriculumName}}</el-checkbox>
              </el-checkbox-group>
            </p>
          </li>
        </ul>
    ...
    <div class="content-box">
      ...
      <ul class="set">
        ...
        <li>
          ...
          <!--<span class="nullInfo" v-if="seting.length === 0">暂无数据</span>-->
          <div class="estates" v-if="seting.editionSet === 1" @click="seting.editionSet = 0">已启用</div>
          <div class="button-group">
...
export default {
  ...
  methods: {
    ...
    handleCheckAllChange (event, item) {
      ...
      item.checked = event.target.checked ? item.courses : []
      let a = {}
      a.id = ''
    ...
    chooseCourses (event, item) {
      ...
      this.list.forEach((child, i) => {
        if (item.setSectionId === child.sectionId && item.setGradeId === child.gradeId) {
          child.subjectIds = item.checked.map(n => n.id).join(',')
          b = true
        }
      })
      if (!b) {
    ...
    getCourse () {
      ...
      this.$https.get(`/lessonSet/getCourse?semesterId=${this.semesterId}`)
        .then(res => {
          if (res.data.code === 1) {
            this.checkedCourses = res.data.result
            this.checkedCourses.forEach((item, index) => {
              item.subjectIds = item.subjectIds.split(',')
              this.checkedInfo = []
              item.subjectIds.forEach(subject => {
                let subInfo = {}
                subInfo.subjectIds = subject.split(',')
                subInfo.companyId = item.companyId
                subInfo.setGradeId = item.gradeId
                subInfo.setSectionId = item.sectionId
                this.checkedInfo.push(subInfo)
              })
            })
            for (var i = 0; i < this.checkedInfo.length; i++) {
              for (var j = 0; j < this.checkedInfo[i].subjectIds.length; j++) {
                for (var m = 0; m < this.setInfo.length; m++) {
                  for (var n = 0; n < this.setInfo[m].courses.length; n++) {
                    if (this.checkedInfo[i].subjectIds[j] === String(this.setInfo[m].courses[n].id)) {
                      this.checkedName.push(this.setInfo[m].courses[n].curriculumName)
        ...
        if (res.data.code === 1) {
          ...
          for (let i = 0; i < this.peroid.length; i++) {
            await this.$https.get(`/common/selectGradeClassList?sectionId=${this.peroid[i].id}&semesterId=${this.semesterId}`)
              .then(async res => {
                if (res.data.code === 1) {
                  this.peroid[i].classes = res.data.result
                  for (let j = 0; j < this.peroid[i].classes.length; j++) {
                    ...
                    .then(res => {
                      if (res.data.code === 1) {
                        this.peroid[i].classes[j].subjects = res.data.result
                        ...
                        this.setInfo.push({ setSectionId: this.setSectionId, setSectionName: this.setSectionName, setGradeId: this.setGradeId, setGradeName: this.setGradeName, courses: this.courses, checkAll: false, isIndeterminate: true, checked: [] })
                      } else if (res.data.code === -2) {
                        error(this)
                      } else {
                        notice(this, res.data.message, 3000)
