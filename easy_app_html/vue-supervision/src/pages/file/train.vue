<template>
    <div class="train">
      ...
      <el-dialog title="上传文件" :visible.sync="dialogVisible">
          <el-form :model="form" :rules="rules" ref="form" v-if="dialogVisible" v-loading="grloading">
  ...
  export default {
    ...
    methods: {
      ...
      deleteList (id) {
        server.post(`/pxwjController/deleteFile`, {
      ...
      getList () {
        server.get(`/pxwjController/getPage?year=${this.value2}&keyword=${this.filename}&pageNo=${this.currentPage}&pageSize=${this.pageSize}`)
        .then(res => {
          if (res.data.code === 1) {
      ...
      Success (response, fileList) {
        ...
        server.post(`/pxwjController/saveFile`, {
      ...
      getYear () {
        server.get(`/pxwjController/getYear`)
        .then(res => {
          if (res.data.code === 1) {
    ...
    computed: {
      action () {
        const path = document.querySelector('#contextPath').value
        return `${path}/pxwjController/uploadAttachment`
      },
      ...mapGetters([
        'previewUrl',
        'rootUrl',
